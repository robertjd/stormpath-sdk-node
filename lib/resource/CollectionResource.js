...
function ensureInstanceResources(items, InstanceConstructor, dataStore, query) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var converted = require('./ResourceFactory').instantiate(InstanceCtor, obj, query, dataStore);
...
function CollectionResource(/*data, query, InstanceCtor, dataStore*/) {
  ...
  if (data && data.hasOwnProperty('items') && data.items.length > 0) {
    data.items = ensureInstanceResources(data.items, InstanceCtor, dataStore, query);
  }
